...
func newCacheStore(dir string, cacheSize int64, limit, pendingPages int, config *Config) *cacheStore {
	...
	if br < c.freeRatio || fr < c.freeRatio {
		logger.Warnf("no enough space or inodes for caching: (%f, %f) < %f", br, fr, c.freeRatio)
	}
	go c.checkFreeSpace()
...
func newCacheManager(config *Config) CacheManager {
	...
	limit := dirCacheSize / int64(config.PageSize) * 2
	if limit < 1000000 {
		limit = 1000000
	}
	m := &cacheManager{
	...
	pendingPages := config.BufferSize * 2 / 10 / config.PageSize / len(dirs)
	for i, d := range dirs {
