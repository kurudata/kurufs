...
func (f *fileWriter) Write(ctx meta.Context, off uint64, data []byte) syscall.Errno {
	...
	indx := uint32(off >> meta.CHUNKBITS)
	pos := uint32(off & meta.CHUNKMASK)
	for len(data) > 0 {
		n := uint32(len(data))
...
func (f *fileWriter) flush(ctx meta.Context, writeback bool) syscall.Errno {
	...
	for len(f.chunks) > 0 && err == 0 {
		...
		if f.flushcond.WaitWithTimeout(time.Millisecond*100) && ctx.Canceled() {
			err = syscall.EINTR
			break
		}
