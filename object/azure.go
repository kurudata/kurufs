...
func (b *abs) Put(key string, data io.ReadSeeker) error {
...
func (b *abs) Delete(key string) error {
	_, err := b.container.GetBlobReference(key).DeleteIfExists(nil)
	return err
}
...
func (b *abs) List(prefix, marker string, limit int64) ([]*Object, error) {
	...
	for err == nil && len(resp.Blobs) == 0 && resp.NextMarker > marker {
		resp, err = b.container.ListBlobs(storage.ListBlobsParameters{
			Prefix:     prefix,
