...
func (q *qiniu) download(key string, off, limit int64) (io.ReadCloser, error) {
	...
	url := storage.MakePrivateURL(q.mac, os.Getenv("QINIU_DOMAIN"), key, deadline.Unix())
...
func (q *qiniu) Get(key string, off, limit int64) (io.ReadCloser, error) {
	if os.Getenv("QINIU_DOMAIN") != "" {
		r, err := q.download("/"+key, off, limit)
		if err == nil {
			return r, nil
		}
	}
...
func (q *qiniu) Put(key string, in io.ReadSeeker) error {
